...
- ✅ **#8 (ogl)** Deferred the WebGL waveform (`ogl`) into its own client-only chunk via `next/dynamic({ ssr: false })` in `VoiceStrands`. `VoiceControl` renders it on nearly every screen, so a static import had been forcing the whole WebGL renderer into the shared bundle; it now loads only on the WebGL2 branch (the non-WebGL2 `VoiceStrand` fallback stays in the critical path).

  _Note on the originally-planned VoiceProvider deferral (#3 rest):_ investigated and **intentionally not done** — the STT/TTS/LLM module graph `VoiceProvider` imports is already light. Every heavy dependency (kokoro, `@huggingface/transformers`, onnxruntime, the Azure Speech SDK, VAD, opencv, tesseract, pdf.js) is already behind a dynamic `import()`; only small orchestration modules are static. Refactoring the 1,100-line provider would risk the core voice flow for little bundle gain, so the higher-value, lower-risk `ogl` deferral was done instead.
